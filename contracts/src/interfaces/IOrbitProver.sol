...
import {WebProof} from "vlayer-0.1.0/WebProof.sol";
...
interface IOrbitProver {
    ...
    error InsufficientDelegationForTier(uint256 requested, uint256 required, uint256 actual);
     ...
     */
    function getTierForAmount(uint256 amount) external pure returns (uint256 tier);
     ...
     */
    function getThresholdForTier(uint256 tier) external pure returns (uint256 threshold);
     ...
     */
    function qualifiesForTier(uint256 amount, uint256 tier) external pure returns (bool qualified);
